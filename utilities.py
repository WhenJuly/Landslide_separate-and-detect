...
def downsample_series(time, series, f_downsample, lowcut=0.5, highcut=5):
    """Down sample the time series given a lower sampling frequency f_downsample,
    time_new, series_downsample, dt_new = downsample_series(time, series, f_downsample)

    The time series has been bandpass filtered (default: 0.5-5Hz) first,
    and then downsampled through interpolation.
    """
    dt = time[1] - time[0]
    fs = 1 / dt
    ...
    # Bandpass filter
    series_filt = bandpass_filter(series, lowcut=lowcut, highcut=highcut, fs=fs)
